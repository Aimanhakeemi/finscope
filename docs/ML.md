...
- Detection is restricted to bill-like categories: `subscriptions`, `utilities`,
  `rent_mortgage`, `health`, and `income`; variable consumer spending such as
  fuel, groceries, dining, coffee, transport, and shopping is not treated as a
  bill even when its timing and amounts look regular.
